...
function gitHookBody(
  ...
  return [
    ...
    '',
    '# Only emit the author block when user.email is set. The server schema',
    '# rejects empty `canonicalEmail`, and `GitEventSchema.author` is optional,',
    '# so omitting it is the correct behavior on an unconfigured workstation.',
    'AUTHOR_FIELD=""',
    'if [ -n "$EMAIL" ]; then',
    '  AUTHOR_FIELD=",\\"author\\":{\\"canonicalEmail\\":\\"$(escape "$EMAIL")\\",\\"displayName\\":\\"$(escape "$NAME")\\",\\"aliases\\":[]}"',
    'fi',
    '',
    'BODY=$(cat <<JSON',
    '{',
    ...
    '  "timestamp": "$(date -u +%Y-%m-%dT%H:%M:%SZ)"$AUTHOR_FIELD',
    '}',
    'JSON',
    ')',
    '',
