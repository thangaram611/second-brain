...
import type { PendingRelation } from '../../pipeline/types.js';
...
function walkGoNode(node: Parser.SyntaxNode, results: ExtractedGoSymbol[]): void {
  switch (node.type) {
    ...
    if (child.type === 'import_declaration') {
      ...
      for (const spec of child.namedChildren) {
        if (spec.type === 'import_spec') {
          ...
          for (const innerSpec of spec.namedChildren) {
