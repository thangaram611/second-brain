...
import type { Collector, ExtractionResult, PendingRelation, PipelineConfig } from '../pipeline/types.js';
import type { LLMExtractor } from '../extraction/llm-extractor.js';
...
export class GitHubCollector implements Collector {
  ...
  async collect(config: PipelineConfig): Promise<ExtractionResult> {
    ...
    try {
      ...
      const { data: rawIssues } = await octokit.issues.listForRepo({
        owner,
        repo: name,
        state: this.options.state,
        per_page: perPage,
